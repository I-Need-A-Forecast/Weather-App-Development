...
##            print(is_between(bounds[b_num - 1], line_seg, b_check))
            ...
            if ((is_between(bounds[b_num - 1], point_n_box[1][seg_num - 1], b_check))
                ...
                print(bounds[b_num - 1], b_check, point_n_box[1][seg_num - 1], line_seg)
                uncovered.append((point_n_box[1][seg_num - 1], line_seg))

temp_uncovered = []

while True:
    temp_len = len(uncovered)
    #print("Looping again!")
    for uncovered_num, uncovered_steps in enumerate(uncovered):
        #print(uncovered[uncovered_num - 1], uncovered_steps)
        #print(uncovered[uncovered_num - 1][1][0], uncovered_steps[0][0])
        if (((round(uncovered[uncovered_num - 1][1][0], z) == round(uncovered_steps[0][0], z)) and
             (round(uncovered[uncovered_num - 1][1][1], z) == round(uncovered_steps[0][1]))) and
             (is_between(uncovered[uncovered_num - 1][0], uncovered_steps[0], uncovered_steps[1]))):

            #print("Same numbers!:", uncovered[uncovered_num - 1], uncovered_steps)
            #print("New vector!:", uncovered[uncovered_num - 1][0], uncovered_steps[1])
            if (uncovered[uncovered_num - 1][0] == temp_uncovered[-1][0]):
                #print("-1 stuff:", temp_uncovered[-1])
                del(temp_uncovered[-1])
            temp_uncovered.append((uncovered[uncovered_num - 1][0], uncovered_steps[1]))
            #print("Check temp uncovered:", temp_uncovered)
        else:
            temp_uncovered.append(uncovered_steps)

    if temp_len == len(uncovered):
        #print("break out")
        break

print("temp uncovered:", temp_uncovered)
print("")
print("TESTING")
print("")
uncovered = temp_uncovered

delete_list = []

for b_num, b_check in enumerate(bounds):
    for uncovered_num, uncovered_steps in enumerate(uncovered):
        if ((round(bounds[b_num - 1][0], z) == round(uncovered_steps[0][0], z)) and
            (round(bounds[b_num - 1][1], z) == round(uncovered_steps[0][1], z)) and
            (round(b_check[0], z) == round(uncovered_steps[1][0], z)) and
            (round(b_check[1], z) == round(uncovered_steps[1][1], z))):

            print("uncovered_num:", uncovered_num, bounds[b_num - 1], b_check, uncovered_steps)
            delete_list.append(uncovered_num)
...
print("Updated list of points left to be assigned:", uncovered)

uncovered_shrinking = []

for b_num, b_check in enumerate(bounds):
    for uncovered_steps in uncovered:

        if ((is_between(bounds[b_num - 1], uncovered_steps[0], b_check)) and
            (is_between(bounds[b_num - 1], uncovered_steps[1], b_check))):
            print((bounds[b_num - 1], uncovered_steps[0], b_check))
            print((bounds[b_num - 1], uncovered_steps[1], b_check))
            if ((round(bounds[b_num - 1][0], z) == round(uncovered_steps[0][0], z)) and
                (round(bounds[b_num - 1][1], z) == round(uncovered_steps[0][1], z))):

                uncovered_shrinking.append((uncovered_steps[1], b_check))

            elif ((round(uncovered_steps[1][0], z) == round(b_check[0], z)) and
                  (round(uncovered_steps[1][1], z) == round(b_check[1], z))):

                uncovered_shrinking.append((bounds[b_num - 1], uncovered_steps[0]))
...
#print("uncovered_shirnking:", uncovered_shrinking)

uncovered = uncovered_shrinking

print("uncovered:", uncovered)

del(uncovered_shrinking)
...
for uncovered_num, uncovered_steps in enumerate(uncovered):
    ...
    print(uncovered_steps)
    midpoint = (((uncovered_steps[0][0] + uncovered_steps[1][0]) / 2.),
                ((uncovered_steps[0][1] + uncovered_steps[1][1]) / 2.))

    for main in points:
        for b_num, b_check in enumerate(bounds):
            print("Look here:", midpoint, main, bounds[b_num - 1], b_check)
            check_point, check_distance = point_intersection(midpoint, main, bounds[b_num - 1], b_check)
            print("Alrighty... ", check_point, check_distance)
            if ((check_point != (None, None)) and
                ((epsilon * -1) <= check_distance <= epsilon)):
                distance = (((check_point[0] - main[0])*(check_point[0] - main[0])) +
                            ((check_point[1] - main[1])*(check_point[1] - main[1]))) ** (1./2)
                print("")
                print(check_point, check_distance)
                holding_array[uncovered_num].append((uncovered_steps, check_point, distance, main))
                print("Ho0lding array:", holding_array, len(holding_array[uncovered_num]))

                if len(holding_array[uncovered_num]) == 2:
                    
                        if holding_array[uncovered_num][0][2] < holding_array[uncovered_num][1][2]:
                           del(holding_array[uncovered_num][1])
                           print("&&&&&&&&&********", holding_array)
                        else:
                            del(holding_array[uncovered_num][0])
                            print("(((())))))", holding_array)


print(holding_array[0][0][3])

for holding_array_step in holding_array:
    for check_all_polygons in all_polygons:
        print(holding_array_step[0][3], check_all_polygons[0])
        if holding_array_step[0][0][3] == check_all_polygons[0]:
            #print(all_polygons[0].index(rebuild[0][3]))
            print(holding_array_step[0][3], check_all_polygons[0])
